...
def create_project():
    ...
    if not os.path.exists(ui_path):
        shutil.copy(
            os.path.abspath(os.path.join(os.path.dirname(__file__), "components/uis/interface.ui")),
    ...
    while True:
        ...
        if (
            appQtBinding != "PySide6"
            and appQtBinding != "PySide2"
            and appQtBinding != "PyQt6"
            and appQtBinding != "PyQt5"
        ):
            print(
                colored(
                    textwrap.dedent(appQtBinding + " is not a valid qt app Qt binding/API Name"),
                    "red",
                )
            )
            continue
        if query_yes_no(
            colored(
                textwrap.dedent(
                    "Your Qt App Qt binding/API Name is " + str(appQtBinding) + ".  Continue?"
                ),
                "blue",
            )
        ):
    ...
    if not os.path.exists(main_py):
        shutil.copy(
            os.path.abspath(os.path.join(os.path.dirname(__file__), "components/python/main.py")),
            os.getcwd(),
        )
    ...
    while True:
        ...
        if query_yes_no(
            colored(
                textwrap.dedent(
                    "Your icons color is " + str(iconsColor) + ". Save the color and continue?"
                ),
                "blue",
            )
        ):
    ...
    while True:
        ...
        if query_yes_no(
            colored(
                textwrap.dedent(
                    "Your icons color is " + str(qtIconsColor) + ". Save the color and continue?"
                ),
                "blue",
            )
        ):
    ...
    while True:
        ...
        if query_yes_no(
            colored(
                textwrap.dedent(
                    "Your app background color is "
                    + str(bgColor)
                    + ". Save the color and continue?"
                ),
                "blue",
            )
        ):
    ...
    while True:
        ...
        if query_yes_no(
            colored(
                textwrap.dedent(
                    "Your app text color is " + str(txtColor) + ". Save the color and continue?"
                ),
                "blue",
            )
        ):
    ...
    while True:
        ...
        if query_yes_no(
            colored(
                textwrap.dedent(
                    "Your app accent color is " + str(accColor) + ". Save the color and continue?"
                ),
                "blue",
            )
        ):
    ...
    while True:
        ...
        if query_yes_no(
            colored(
                textwrap.dedent("Your app name is " + appName + ". Save the name and continue?"),
                "blue",
            )
        ):
            break
    ...
    while True:
        organizationName = input(
            textwrap.dedent("Please enter the your organization name (Optional): ")
        )
        ...
        if query_yes_no(
            colored(
                textwrap.dedent(
                    ...
                    + organizationName
                    + ". Save the organization name and continue?"
                ),
                "blue",
            )
        ):
    ...
    while True:
        ...
        if query_yes_no(
            colored(
                textwrap.dedent(
                    "Your domain name is " + domainName + ". Save the domain name and continue?"
                ),
                "blue",
            )
        ):
    ...
    if not os.path.exists(json_path):
        shutil.copy(
            os.path.abspath(os.path.join(os.path.dirname(__file__), "components/json/style.json")),
            os.path.join(os.getcwd(), "json-styles"),
        )
    ...
    if not os.path.exists(readme_path):
        shutil.copy(
            os.path.abspath(os.path.join(os.path.dirname(__file__), "components/md/README.md")),
            os.getcwd(),
        )
    ...
    NewIconsGenerator.generateIcons(
        progress_callback, normal_color, "", normal_color.replace("#", "")
    )
